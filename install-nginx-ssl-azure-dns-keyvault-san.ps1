...
"delay: $($delay * 500)" | out-file -append c:\temp\delay.txt
"before delay: $(get-date)" | out-file -append c:\temp\delay.txt
Start-Sleep ($delay * 500)
...
if ($Staging -eq "disabled" ) {
        ...
        $certificatename = (($HostNamesArray[0].Replace("*","wildcard")).Replace(".","-"))
        $pemfilesname = "$($HostNamesArray[0])"
    }
    else {
        $baseUri = "https://acme-staging-v02.api.letsencrypt.org"
        $certificatename = (($HostNamesArray[0].Replace("*","wildcard")).Replace(".","-")) + '-test'
        $pemfilesname = "$($HostNamesArray[0])"
        
    }

e:\simple-acme\wacs.exe --baseuri $baseUri --verbose `
 --accepttos --emailaddress noreply@noreply.org --source manual --host $HostNames --validationmode dns-01 --validation azure  `
...
http {
    ...
    server {
        ...
        ssl_certificate      ssl/$($HostNamesArray[0].Replace('*','_'))-chain.pem;
        ssl_certificate_key  ssl/$($HostNamesArray[0].Replace('*','_'))-key.pem;
